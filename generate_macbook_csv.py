...
# Generate Main Catalog CSV
csv_main_headers = [
    "Product Name", "Brand", "Category", "Base Price (AED)", 
    "Color Options", "Specification Variants & Price Boosts", "Short Description"
]

target_dir_1 = r"c:\Users\shant\sellphone.ae"
target_dir_2 = r"C:\Users\shant\.gemini\antigravity-ide\brain\ceae0dbf-d5b8-4d55-b6c1-296d4da1afdd"

for dest_dir in [target_dir_1, target_dir_2]:
    os.makedirs(dest_dir, exist_ok=True)
    
    # 1. Main Catalog CSV
    main_csv_path = os.path.join(dest_dir, "Apple_MacBook_Laptops_Catalog.csv")
    with open(main_csv_path, mode="w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(csv_main_headers)
    ...
    print(f"Created CSV: {main_csv_path}")

    # 2. Itemized Breakdown CSV
    itemized_csv_path = os.path.join(dest_dir, "Apple_MacBook_Spec_Variants_Breakdown.csv")
    csv_itemized_headers = [
        "Product Model Name", "Brand", "Category", "Base Price (AED)",
        "Variant Spec / UOM (RAM & Storage)", "Spec Boost (AED)", "Total Valuation (AED)", "Colors"
    ]
    with open(itemized_csv_path, mode="w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(csv_itemized_headers)
        for p in macbook_products:
            for v in p["variants"]:
    ...
    print(f"Created CSV: {itemized_csv_path}")
